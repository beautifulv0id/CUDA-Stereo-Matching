...
)
{
    ...
    for ( int idx = getGlobThreadIdx(); idx < splitTriArr._num; idx += getThreadNum() )
    {
...
{
    for ( int idx = getGlobThreadIdx(); idx < actTriNum; idx += getThreadNum() )
    {
        int output = -1; 
...
{
    // Iterate flips
    for ( int flipIdx = getGlobThreadIdx(); flipIdx < flipToTri._num; flipIdx += getThreadNum() )
    {
        int botIdx, botVi;  
...
{
    // Iterate flips
    for ( int flipIdx = getGlobThreadIdx(); flipIdx < flipNum; flipIdx += getThreadNum() )
    {
        int botIdx, botVi;  
...
{
    for ( int idx = getGlobThreadIdx(); idx < flipNum; idx += getThreadNum() )
    {
...
{
    for ( int idx = getGlobThreadIdx(); idx < triVec._num; idx += getThreadNum() )
    {
...
{
    for ( int idx = getGlobThreadIdx(); idx < idxVec._num; idx += getThreadNum() )
    {
...
{
    for ( int idx = getGlobThreadIdx(); idx < triInfoVec._num; idx += getThreadNum() )
    {
...
)
{
    ...
    for ( int idx = getGlobThreadIdx(); idx < noSample; idx += getThreadNum() )
    {
...
{
    for ( int idx = getGlobThreadIdx(); idx < newTriNum; idx += getThreadNum() )
    {
        if ( isTriAlive( triInfoArr[ idx ] ) ) continue; 
...
{
    for ( int idx = newTriNum + getGlobThreadIdx(); idx < triInfoVec._num; idx += getThreadNum() )
    {
...
{
    for ( int idx = getGlobThreadIdx(); idx < actTriVec._num; idx += getThreadNum() )
    {
        int output = -1; 
